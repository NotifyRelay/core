...
impl MdnsState {
    ...
    pub fn start_advertiser(
        ...
        let daemon = ServiceDaemon::new()
            .map_err(|e| format!("创建 mDNS 服务守护进程失败: {}", e))?;
    ...
    pub fn start_browser(
        ...
        let daemon = ServiceDaemon::new()
            .map_err(|e| format!("创建 mDNS 浏览守护进程失败: {}", e))?;
                            ...
                            match event {
                                ServiceEvent::ServiceResolved(info) => {
                                    ...
                                    if let Some(cb) = on_discovered_cb {
                                        let uuid_c =
                                            std::ffi::CString::new(uuid).unwrap_or_default();
                                        let name_c =
                                            std::ffi::CString::new(name).unwrap_or_default();
                                        let ip_c =
                                            std::ffi::CString::new(ip).unwrap_or_default();
                                        let dt_c =
                                            std::ffi::CString::new(dt).unwrap_or_default();
                                        cb(
                                            uuid_c.as_ptr(),
                                            name_c.as_ptr(),
                                            ip_c.as_ptr(),
