...
pub unsafe extern "C" fn nrc_start_mdns_advertiser(
    ...
    let pk = from_cstr(pubkey).to_string();
...
#[no_mangle]
pub unsafe extern "C" fn nrc_stop_mdns_advertiser(ctx_ptr: *mut c_void) -> i32 {
    if ctx_ptr.is_null() {
        return -1;
    }
    let ctx = &mut *(ctx_ptr as *mut crate::SafeContext);
...
pub unsafe extern "C" fn nrc_start_mdns_discovery(ctx_ptr: *mut c_void) -> i32 {
    ...
    match guard
        .mdns
        .start_browser(ctx_ptr_usize, on_mdns_discovered, user_data)
    {
        Ok(_) => 0,
        Err(e) => {
...
#[no_mangle]
pub unsafe extern "C" fn nrc_stop_mdns_discovery(ctx_ptr: *mut c_void) -> i32 {
    if ctx_ptr.is_null() {
        return -1;
    }
    let ctx = &mut *(ctx_ptr as *mut crate::SafeContext);
